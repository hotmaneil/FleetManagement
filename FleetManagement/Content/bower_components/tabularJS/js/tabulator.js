...
;(function (global, factory) {
	...
	Download.prototype.downloaders = {
		...
		string: function xlsx(columns, data, options, setFileContents) {
		    var fileContents = JSON.stringify(data, null, '\n');
		    var fileContents = fileContents.toString();
		    setFileContents(fileContents, "application/txt;charset=utf-8");
		},
		pdfmake: function xlsx(columns, data, options, setFileContents) {
		    var self = this,
                fields = [],
                header = [],
                setWitths = [],
                ...
                autoTableParams = {},
                title = options && options.title ? options.title : "",
                orientation = options && options.orientation == "portrait" ? "portrait" : "landscape";

		    //build column headers
		    columns.forEach(function (column) {
		        if (column.field) {
		            var styleColumn = { text: column.title || "", bold: true, fillColor: "#97deb1" };
		            // header.push(column.title || "");
		            header.push(styleColumn);
		            fields.push(column.field);
		        }
		    });
		    body.push(header);
		    ...
		    var docDefinition = [
                {
                    ...
                    footer: function (currentPage, pageCount) {
                        columns: [
                           {
                               alignment : 'center',
                               text      : '第'+currentPage.toString() + '頁/共' + pageCount.toString()+'頁',
                               fontSize  : 8
                           }
                        ]
                    },
                            ...
                            table: {
                                ...
                                layout: {
                                    ...
                                    hLineColor: function (i, node) {
                                        return (i === 0 || i === node.table.body.length) ? '#000000' : '#888888';
                                    },
                                    vLineColor: function (i, node) {
                                        return (i === 0 || i === node.table.widths.length) ? '#000000' : '#888888';
                                    },
                                }
                            }
                        }
		    ...
		    var pdf = options.pdfMake;
		    //var font = options.pdfFonts;
		    options.filename = (options.filename == "" || typeof options.filename == "undefined") ? "report.pdf" : options.filename + ".pdf";
		    //pdf.vfs = options.vfs.Returnvfs();
		    pdf.vfs = options.vfs;
		    for(var i=0;i< docDefinition.length;i++){
		        var typedDefinition=docDefinition[i];
		        pdfMake.createPdf(typedDefinition).download(options.filename);
		    }
		}
